...
router.get('/', authenticate, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const moods = await MoodModel.findByDateRange(req.userId!, startDate, endDate)
  res.json({ data: moods })
}))
...
router.get('/today', authenticate, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const mood = await MoodModel.findByDate(req.userId!, today)
...
router.get('/stats', authenticate, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const stats = await MoodModel.getStats(req.userId!, days)
  res.json({ data: stats })
}))
...
router.post('/', authenticate, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const mood = await MoodModel.upsert(req.userId!, data)
  res.json({ data: mood })
}))
